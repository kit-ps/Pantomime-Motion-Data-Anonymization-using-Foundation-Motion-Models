...
def classify(data, labels, split_labels=False):
    ...
    k_fold_strat = StratifiedKFold(n_splits=5)
    ...
    for i in range(len(y_test)):
        if y_test[i] not in score_voting:
            score_voting[y_test[i]] = [predictions[i]]
        else:
            score_voting[y_test[i]].append(predictions[i])
    ...
    return {"validation" : validation_score, "classification": classification_score, "predictions": predictions,
            "true_labels": y_test, "classification_voting": voting_score}
...
def run_verification_for_anon(args):
    ...
    prior_results = {}
    print(data_import_dir + "result.pickle")
    results_pickle_path = join(data_import_dir, "results.pickle")
...
def run_verification_experiments(input, attributes):
    ...
    if len(input) == 2:
        ...
        #args = ("../data/prepared/", None, ["original_positions", "vposer_positions", "humor_positions"], True)
        '''attr = ['original_positions', 'vposer_betas', 'vposer_trans', 'vposer_root_orient',
                ...
                'vposer_positions_without_betas_root_orient_pose_body', 'vposer_SMPL', 'vposer_SMPL_vposer',
                'vposer_SMPL_humor', 'humor_betas', 'humor_trans', 'humor_root_orient', 'humor_pose_body',
                'humor_latent_motion', 'humor_contacts', 'humor_latent_pose', 'humor_positions',
                ...
                'humor_positions_without_betas_root_orient_pose_body', 'humor_SMPL', 'humor_SMPL_vposer',
                'humor_SMPL_humor']'''
        attr = ['original_positions', 'vposer_betas', 'vposer_trans', 'vposer_root_orient',
                'vposer_pose_body', 'vposer_latent_pose', 'vposer_latent_motion', 'vposer_positions',
                'vposer_positions_without_trans_betas_root_orient', 'vposer_positions_without_trans_betas_pose_body',
        ...
        attr = ['vposer_positions_without_trans_betas_root_orient', 'vposer_positions_without_trans_betas_pose_body',
                'vposer_positions_without_trans_root_orient_pose_body',
                'vposer_positions_without_betas_root_orient_pose_body', 'humor_positions_without_trans_betas_root_orient', 'humor_positions_without_trans_betas_pose_body',
                'humor_positions_without_trans_root_orient_pose_body',
                'humor_positions_without_betas_root_orient_pose_body']
        ...
        attr_reduced_reduced = ["original_positions"]
        args = ("../data/prepared_extra_experiments/", None, attr, True)
        print("foo")
