...
def triplet_loss(anchor, positive, negative, *args):
    tl = nn.TripletMarginLoss(margin=args[0]["margin"], p=2, eps=1e-7)
    return tl(anchor, positive, negative)


def triplet_batch_loss(output, target, *args):
    tl = BatchAllTtripletLoss(margin=args[0]["margin"])
    return tl(output, target)
...
class BatchAllTtripletLoss(nn.Module):
    ...
    #eps is an arbitrary small value to be used for numerical stability tricks
    ...
    def euclidean_distance_matrix(self, x, y=None):
        '''
    ...
    def euclidean_distance_matrix(self, x):
        ...
        distance_matrix += mask * self.eps

        zero_indices = distance_matrix < self.eps
        distance_matrix[zero_indices] = self.eps
    ...
    def forward(self, embeddings, labels):
        ...
        # shape: (batch_size, batch_size, batch_size)
        mask = BatchAllTtripletLoss.get_triplet_mask(labels)
        triplet_loss *= mask
